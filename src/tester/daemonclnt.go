...
// path to daemon binary: prefer src/bin/<prog> (go build -o bin/kvsrv1d ./cmd/kvsrv1d),
// fall back to src/main/<prog> for legacy MIT lab layouts.
func path(prog, cwd string) string {
	i := strings.LastIndex(cwd, "src")
	root := filepath.Join(cwd[:i], "src")
	for _, dir := range []string{"bin", "main"} {
		p := filepath.Join(root, dir, prog)
		if st, err := os.Stat(p); err == nil && !st.IsDir() {
			return p
		}
	}
	return filepath.Join(root, "bin", prog)
}
